...
LL custom_hash(LL L)
{
    if(L == 1) return res[0]%M;
    return (custom_hash(L-1) * 100001 + res[L-1]) %M;
}
...
int solve(int testcase)
{
    ...
    reverse(all(inp));
    for(auto x : inp)
        (x == '0' ? init.push_back(-1) : init.push_back(1));
    for(int i = 0 ; i < q ; i++)
    {
        ...
        cin >> que;
        for(auto x : que)
            (x == '0' ? res.push_back(-1) : res.push_back(1));
        // fft
        s = Mul(res, init);
        res.clear();
